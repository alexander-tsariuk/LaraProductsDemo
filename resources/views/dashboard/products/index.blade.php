...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 col-lg-10 mb-3">
            <a href="{{ route("$route.create") }}" class="btn btn-primary">Добавить</a>
        </div>
    </div>
            ...
            <tbody>
                @forelse($items as $item)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('products.edit', ['product' => $item->id]) }}">Редактировать</a>
                            <form method="post" action="{{ route('products.destroy', ['product' => $item->id]) }}" id="delete-row-{{ $item->id }}">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                ...
                @empty
                    <tr>
                        <td class="text-center" colspan="8">Ни одного товара не было добавлено</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
